...
app.post('/register', async (req, res) => {
    ...
    // Email validation
    const emailPattern = /@(spelman\.edu)$/;
    if (!emailPattern.test(email)) {
        return res.status(400).json({ success: false, message: 'Email must end with @spelman.edu' });
    }
...
app.post('/api/profile', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from('profiles')
      .upsert([{ user_id: id, name, email }], { onConflict: 'user_id' })
      .select('*')
      .single();
